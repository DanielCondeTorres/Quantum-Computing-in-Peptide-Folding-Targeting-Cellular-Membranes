...
class MainBead(BaseBead):
    ...
    def _build_turn_indicator_fun_0(self) -> OperatorBase:
        return (self._full_id^ ((self._full_id - self._turn_qubits[0])@ (self._full_id - self._turn_qubits[1]))).reduce()

    def _build_turn_indicator_fun_1(self) -> OperatorBase:
        return (self._full_id^ (self._turn_qubits[1] @ (self._turn_qubits[1] - self._turn_qubits[0]))).reduce()

    def _build_turn_indicator_fun_2(self) -> OperatorBase:
        return (self._full_id^ (self._turn_qubits[0] @ (self._turn_qubits[0] - self._turn_qubits[1]))).reduce()
