...
class DistanceMap:
    ...
    def __init__(self, peptide: Peptide):
        ...
        self._distance_map, self._num_distances, = DistanceMapBuilder().create_distance_qubits(peptide)
    ...
    def displacement_map(self, axis=0): # this is to get the displacement map, by default along the axis 0
        """Returns a displacement map between beads in the dimension of one axis"""
        displacements, num_displacements = DistanceMapBuilder().create_distance_qubits(self.peptide, axis)
        return displacements

    def first_neighbor(self, peptide: Peptide, lower_bead_ind: int, is_side_chain_lower: int, upper_bead_ind: int, is_side_chain_upper: int,
        lambda_1: float, pair_energies: np.ndarray, adjustment_mj: bool = False  , energy_mod_mj: [PauliSumOp, PauliOp] = 0 ,  pair_energies_multiplier: float = 0.1 ) -> Union[PauliSumOp, PauliOp]:
        """
        ...
        if adjustment_mj == False:
            expression = lambda_0 * (x - _build_full_identity(x.num_qubits)) + pair_energies_multiplier * energy * _build_full_identity(x.num_qubits) 
        else:
    ...
    def second_neighbor(self, peptide: Peptide, lower_bead_ind: int, is_side_chain_lower: int, upper_bead_ind: int, is_side_chain_upper: int,
        lambda_1: float, pair_energies: np.ndarray, adjustment_mj: bool = False,   energy_mod_mj: [PauliSumOp, PauliOp] = 0  ,pair_energies_multiplier: float = 0.1 ) -> Union[PauliSumOp, PauliOp]:
