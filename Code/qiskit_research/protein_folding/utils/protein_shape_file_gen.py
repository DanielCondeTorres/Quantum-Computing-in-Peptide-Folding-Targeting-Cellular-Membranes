...
class ProteinShapeFileGen:
    ...
    def plane_equation(self):
        ...
        axis = interface_parameters.axis 
        ...
        print('Interface :',interface_parameters().get_weight_interface) 
        ...
        if interface_parameters().get_weight_interface != 0:
            try:
                ...
                x0, y0, z0 = self._main_positions[0]+self._main_positions[1]*interface_parameters.displacement_of_the_interface_plane
        ...
        # No plane to represent
            ...
            ABCD =[A,B,C,D]
            changing_the_plane_by_points = np.array([0]*4)
        print('CHANGING', changing_the_plane_by_points)
    ...
    def main_positions(self) -> np.ndarray:
        ...
        """
        print('relative positions main chain aa: ',self._main_positions)
    ...
    def save_xyz_file(
        ...
        ''' 
        EXAMPLE*: Add the plane in the .xyz file
        
        data_plane = self._data_plane()
        data = np.vstack((data,data_plane))
        
        '''
        number_of_particles = data.shape[0]
        header = f"{number_of_particles}\n{comment}"
        np.savetxt(fname=file_path,header=header,X=data,delimiter=" ",fmt="%s",comments="")
        ...
        if name == 'STABLE' and not name == 'plane_vmd':
            ...
            name_vmd = 'plane_vmd'
            header=f"{number_of_particles}\n{comment}"
            path_vmd_representation= os.path.join(path,name_vmd + ".xyz")
            np.savetxt(
                fname=path_vmd_representation,
                header=header,
                X=data_new,
                delimiter=" ",
                fmt="%s",
